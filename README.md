...
- [API Gateway & IAM](#api-gateway--iam)
    ...
    - [Architecture du projet](#architecture-du-projet)
        ...
        - [Authentification et autorisation avec Keycloak](#authentification-et-autorisation-avec-keycloak)
            - [Autorisation des accès avec OAuth 2.0](#autorisation-des-acc%C3%A8s-avec-oauth-20)
    ...
    - [Prérequis pour la suite](#pr%C3%A9requis-pour-la-suite)
    - [Comment utiliser ce projet ?](#comment-utiliser-ce-projet-)
        - [Récupérer le projet Github](#r%C3%A9cup%C3%A9rer-le-projet-github)
        - [Créer un cluster K8S en local](#cr%C3%A9er-un-cluster-k8s-en-local)
            - [Avec Minikube conseillé](#avec-minikube-conseill%C3%A9)
            - [Avec Kind](#avec-kind)
        - [Créer d'un namespace demo](#cr%C3%A9er-dun-namespace-demo)
        - [Déployer un network policy optionnel](#d%C3%A9ployer-un-network-policy-optionnel)
        - [Déployer les stacks monitoring et tracing](#d%C3%A9ployer-les-stacks-monitoring-et-tracing)
        - [Déployer **KrakenD**](#d%C3%A9ployer-krakend)
        - [Déployer **Keycloak**](#d%C3%A9ployer-keycloak)
        - [Déployer la boutique demo Google](#d%C3%A9ployer-la-boutique-demo-google)
        - [Mettre à jour le fichier /etc/hosts](#mettre-%C3%A0-jour-le-fichier-etchosts)
    - [Sécurisation des application et des APIs REST avec Keycloak](#s%C3%A9curisation-des-application-et-des-apis-rest-avec-keycloak)
        - [Etape 1: Ajouter un realm nommé demo](#etape-1-ajouter-un-realm-nomm%C3%A9-demo)
        - [Etape 2: Ajouter un utilisateur demo](#etape-2-ajouter-un-utilisateur-demo)
        - [Etape 3: Autoriser notre application front à consommer les endpoints exposés par l'API Gateway](#etape-3-autoriser-notre-application-front-%C3%A0-consommer-les-endpoints-expos%C3%A9s-par-lapi-gateway)
    - [Validation des JWT avec KrakenD](#validation-des-jwt-avec-krakend)
    - [Procédure de tests](#proc%C3%A9dure-de-tests)
        - [Etape 1: Préparer l'environnement](#etape-1-pr%C3%A9parer-lenvironnement)
        - [Etape 2. Récupérer le token d'accès access token](#etape-2-r%C3%A9cup%C3%A9rer-le-token-dacc%C3%A8s-access-token)
        ...
        - [Visualer les métriques de l'API Gateway](#visualer-les-m%C3%A9triques-de-lapi-gateway)
        - [Consulter les traces des requêtes exécutées](#consulter-les-traces-des-requ%C3%AAtes-ex%C3%A9cut%C3%A9es)
    - [Comment arrêter/supprimer le cluster K8S ?](#comment-arr%C3%AAtersupprimer-le-cluster-k8s-)
        - [Pour arrêter](#pour-arr%C3%AAter)
...
* Jaeger
    ...
    ```sh
    kubectl apply -f addons/tracing/jaeger -n monitoring
    ```
